...
If there’s enough space in the slice’s backing array, the element just gets added.
If there’s not enough space, a new array is allocated, all the items are copied over,
and the new item is added at the end. The interesting part is allocating that new array.
...
/*
When you print %p of a slice, it prints the address of it’s backing array
(see here in reflect.Pointer and SliceHeader type). That allows us to see what’s happening
with that array:
...
*/
//At 1024 we grow to 1280 (1024 + 1024/4), not 2048. This all happens in
//pkg/runtime/slice.c, with similar code in reflect/value.go.

//What all this means to me is that if you know even vaguely how big your slice will get,
//always allocate with a capacity (i.e. make([]int, 0, 256)). That will save a lot of
//allocation, copying, and subsequent garbage collection.
